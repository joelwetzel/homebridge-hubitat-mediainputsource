      ...
      status.text('Error');
                ...
                if (req.body.content.deviceId == null) {
                    switch (req.body.content.name) {
                        case 'hsmStatus':
                            ...
                            break;
                        case 'hsmAlert':
                            if ((req.body.content.value === 'cancel') || (req.body.content.value === 'cancelRuleAlerts')) {
                    ...
                    if (jsonData['source'] === 'DEVICE') {
                        ...
                        switch (jsonData['name']) {
                            case 'hsmStatus':
                                ...
                                break;
                            case 'hsmAlert':
                                if ((jsonData['value'] === 'cancel') || (jsonData['value'] === 'cancelRuleAlerts')) {
                                    platform.log('Received HSM Cancel');
                                    newChange.push({
                                        device: 'hsm' + platform.config['name'],
                                        displayName: 'Alarm System ' + platform.config['name'],
            ...
            freePort.getAvailablePort(platform.local_port).then(port => {
                ...
                server.listen(port, function () {
                    platform.log('homebridge-hubitat-mediainputsource server listening on ' + port);
                });
